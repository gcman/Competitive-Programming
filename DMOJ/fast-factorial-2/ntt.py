...
def FFTMult(a,b):
	...
	na = len(a)
	nb = len(b)
	a += [0 for _ in range(nb-1)]
	b += [0 for _ in range(na-1)]
	pad = 2**math.ceil(math.log(len(a),2)) - len(a)
	a += [0 for _ in range(pad)]
	b += [0 for _ in range(pad)]
	c = circular_convolve(a,b)
	c = inverse_transform(c,root,mod)
	return c

def divdown(k,f,M): # Polynomial f of degree less than 2**k, and subproducts M
	n = 2**k
	if n == 1:
		return f
